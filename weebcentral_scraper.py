        ...
        for image_file in image_files:
            try:
                with Image.open(image_file) as img:
                    ...
                    is_double_page = aspect_ratio > 1.4  # tune threshold if needed
                    ...
                    # If it's a double page, rotate it 90° clockwise
                    if is_double_page:
                    ...
                    if width / height > page_w / page_h:
                        new_w = page_w
                        new_h = page_w * height / width
                    else:
                        new_h = page_h
                        new_w = page_h * width / height

                    # Center image on the page
                    x = (page_w - new_w) / 2
                    y = (page_h - new_h) / 2

                    pdf.image(img, x=x, y=y, w=new_w, h=new_h)
            except Exception as e:
                logger.error(f"Failed to process image {image_file}: {e}")

        pdf_path = os.path.join(self.output_dir, f"{"".join(chapter_name.split())}.pdf")
